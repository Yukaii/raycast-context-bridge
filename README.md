...
Load `dist/` as an unpacked extension in Chrome to test. The build currently emits a warning about `eval` inside the user script because the extension allows Raycast to execute snippets sent from the desktop app—this matches the original behavior and should only be changed if you plan to restrict that feature.
